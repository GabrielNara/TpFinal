...
class AdministradorModel
{
    ...
    public function obtenerCantidadJugadoresMesM($mes)
    {
        $query = "SELECT COUNT(*) FROM `usuarios` WHERE  sexo = 'M' and MONTH(fecha_registro) = '$mes'";

        $cantidad = $this->database->querySelectFetchAssoc($query);
        return intval($cantidad[0]["COUNT(*)"]);
    }

    public function obtenerCantidadJugadoresMesF($mes)
    {
        $query = "SELECT COUNT(*) FROM `usuarios` WHERE  sexo = 'F' and MONTH(fecha_registro) = '$mes'";

        $cantidad = $this->database->querySelectFetchAssoc($query);
        return intval($cantidad[0]["COUNT(*)"]);
    }
    public function obtenerCantidadJugadoresMesOtro($mes)
    {
        $query = "SELECT COUNT(*) FROM `usuarios` WHERE  sexo = 'Otro' and MONTH(fecha_registro) = '$mes'";
